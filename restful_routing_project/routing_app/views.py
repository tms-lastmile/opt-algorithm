...
def testing(request, format=None):
    ...
    df_delivery_orders = get_delivery_orders_dataframe(delivery_orders)
    df_do_dest_location = get_location_dataframe(dest_location)
    df_do_origin_location = get_location_dataframe(origin_location['locations'])

    df_raw = pd.merge(df_delivery_orders, df_do_dest_location, on='loc_dest_id', how='left')

    filtered_clustered_origin_loc = pd.concat([pd.DataFrame(df_do_origin_location), df_raw]).drop_duplicates().reset_index(drop=True)
    _, times = get_distance_runner(filtered_clustered_origin_loc)

    print(filtered_clustered_origin_loc)

    time_windows = list(zip(
        filtered_clustered_origin_loc['open_hour'].apply(lambda x: x.hour * 60 + x.minute),
        filtered_clustered_origin_loc['close_hour'].apply(lambda x: x.hour * 60 + x.minute)
    ))
    services_time = list(filtered_clustered_origin_loc['service_time'])
    ...
    unreachable_locations_index = result['unreachable']
    ...
    for index , loc_index in enumerate(reachable_locations_index):
        loc = filtered_clustered_origin_loc.iloc[loc_index]
        prev_loc = filtered_clustered_origin_loc.iloc[prev_loc_index]
    ...
    actual_reachable_locations = filtered_clustered_origin_loc.iloc[actual_reachable_locations_index]
    actual_unreachable_locations = filtered_clustered_origin_loc.iloc[actual_unreachable_locations_index]

    list_of_location_routes = []
    all_coords = []
    prev_locaction = filtered_clustered_origin_loc.iloc[0]
    list_of_location_routes.append({'location_id': prev_locaction['loc_dest_id']})
    ...
    for index, location in actual_reachable_locations.iterrows():
        ...
        for cord in fetch_concatenate_route(location['latitude'], location['longitude'], prev_locaction['latitude'], prev_locaction['longitude']):
            all_coords.append(list(cord))
        prev_locaction = location

    reachable_shipment = {
        "reachable": True,
        "location_routes": [{"location_id": loc_dest_id} for loc_dest_id in actual_reachable_locations['loc_dest_id'].tolist()],
        "delivery_orders": [{"delivery_order_id": delivery_order_id} for delivery_order_id in filtered_clustered_origin_loc[filtered_clustered_origin_loc['loc_dest_id'].isin(actual_reachable_locations['loc_dest_id'])]["id"].tolist()],
    ...
    unreachable_shipment = {
        "reachable": False,
        "delivery_orders": [{"delivery_order_id": delivery_order_id} for delivery_order_id in filtered_clustered_origin_loc[filtered_clustered_origin_loc['loc_dest_id'].isin(actual_unreachable_locations['loc_dest_id'])]["id"].tolist()],
    }

    shipments = [reachable_shipment, unreachable_shipment]

    print("Routing completed.")
    return Response(shipments, 200)
...
def priority_optimization(request, format=None):
    ...
    dest_location = data['dest_location']
    origin_location = data['ori_location']
    origin_latitude = origin_location['locations'][0]['latitude']
    origin_longitude = origin_location['locations'][0]['longitude']
    warehouse = [origin_latitude, origin_longitude]
    delivery_orders = data["delivery_orders"]
    priority = data["priority"]

    print("this is" ,priority)
    ...
    df_do_dest_location = get_location_dataframe(dest_location)
    df_do_origin_location = get_location_dataframe(origin_location['locations'])
    # print("datang ke:", df_do_dest_location )
    # print("datang dari:", df_do_origin_location )
